...
class QCRM_Homepage extends QCRM_Homepage_sugar {
	...
	public function setDefaultHomepage(){
		global $sugar_config;
		
		require_once('custom/modules/Administration/QuickCRM_utils.php');
		$configurator = new Configurator();
		$configurator->loadConfig();
		if (empty($configurator->config['quickcrm_default_homepage']) || $configurator->config['quickcrm_default_homepage'] != $this->id){
			$configurator->config['quickcrm_default_homepage'] = $this->id;
			$configurator->saveConfig();
			// update QuickCRM config
			$sugar_config['quickcrm_default_homepage'] = $this->id;
			$qutils=new QUtils();
			...
			if (!empty($this->id)){
				...
				foreach ($mobile_users as $user_id){
					if ($user_id == $current_user->id){
						...
						$copy_it = true;
						if ($role != ''){
							$copy_it = in_array($role, ACLRole::getUserRoleNames($user_id));
						}
						if ($copy_it){
