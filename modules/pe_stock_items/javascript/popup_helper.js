...
function send_back_selected(module, form, field, error_message, request_data) { 
    ...
    var form_name = request_data.form_name;
    var field_to_name_array = request_data.field_to_name_array;
    var serial_return_id = field_to_name_array.serial_number;

    var serials = Array();
    for (var key in selection_list_array){
        if( typeof associated_javascript_data[selection_list_array[key]].SERIAL_NUMBER !== 'undefined' &&  
        associated_javascript_data[selection_list_array[key]].SERIAL_NUMBER !== "") 
            serials.push (associated_javascript_data[selection_list_array[key]].SERIAL_NUMBER);
    }
    //
    var serials_to_return = Array();
    serials_to_return[serial_return_id] = serials.join(", ");
    SUGAR.util.globalEval("var call_back_function = window.opener." + request_data.call_back_function);
    // parse serial numbers from selection_list_array
    /*
    for (var the_key in field_to_name_array) {
        if (the_key != 'toJSON') {
            if (YAHOO.lang.isArray(field_to_name_array[the_key])) {
