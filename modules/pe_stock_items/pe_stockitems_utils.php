...
function perform_pe_stockitems_save($focus){
    // Check serial number 
    /*if ( isset($focus->serial_number) && $focus->serial_number != "") {
        // Query to get id
        $db = DBManagerFactory::getInstance();
        $sql = "SELECT * FROM pe_stock_items
                WHERE 1=1 
                AND serial_number = '$focus->serial_number' ";
    ...
    /*foreach($focus->field_defs as $field){
        ...
        if(isset($focus->field_defs[$fieldNameDollar])){
            ...
            if(!number_empty($focus->field_defs[$field['name']])){
                ...
                if (!amountToConvertIsDatabaseValueStockItems($focus, $fieldName)) {
                    if (!isset($focus->$fieldName)) {
...
function amountToConvertIsDatabaseValueStockItems($focus, $fieldName)
{
