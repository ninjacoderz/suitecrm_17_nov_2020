...
class templateParserQuoteForm
{
    public static function parse_template_quote_form($string, $bean_arr)
    {
        foreach ($bean_arr as $bean_name => $bean_id) {
            $focus = BeanFactory::getBean($bean_name, $bean_id);
            $string = templateParserQuoteForm::parse_template_bean_quote_form($string, $focus->table_name, $focus);
            ...
            foreach ($focus->field_defs as $focus_name => $focus_arr) {
                if ($focus_arr['type'] == 'relate') {
                    if (isset($focus_arr['module']) && $focus_arr['module'] != '' && $focus_arr['module'] != 'EmailAddress') {
                        ...
                        $string = templateParserQuoteForm::parse_template_bean_quote_form($string, $focus_arr['name'], $relate_focus);
                    }
                }
            }
        }
     ...
     * @throws Exception
     */
    public function parse_template_bean_quote_form($string, $key, &$focus)
    {
        ...
        foreach ($focus->field_defs as $field_def) {
            if (isset($field_def['name']) && $field_def['name'] != '') {
                ...
                if ($field_def['type'] == 'currency') {
                    ...
                    if($focus->{$fieldName} > 0) {
                        $repl_arr[$key . "_" . $fieldName] = '$'.currency_format_number(
                            $focus->{$fieldName},
                            $params
                        );
                    } elseif($focus->{$fieldName} < 0) {
                        $repl_arr[$key . "_" . $fieldName] = str_replace('-','$-',currency_format_number(
                            $focus->{$fieldName},
                            $params
                        ));
                    } else {
