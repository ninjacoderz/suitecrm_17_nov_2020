    ...
    public function save($check_notify = false)
    {
        ...
        if (empty($this->id)) {
            if ($sugar_config['dbconfig']['db_type'] == 'mssql') {
                $this->number = (int)$this->db->getOne("SELECT MAX(CAST(number as INT))+1 FROM accounts");
            } else {
                $this->number = (int)$this->db->getOne("SELECT MAX(CAST(number as UNSIGNED))+1 FROM accounts");
            }
        }
