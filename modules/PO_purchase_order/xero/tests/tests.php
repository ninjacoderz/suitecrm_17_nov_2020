...
if (isset($_REQUEST['invoice'])) {
    ...
    if (isset($_REQUEST['method']) && $_REQUEST['method'] == "put" && $_REQUEST['invoice']== 1 ) {
        ...
        // Tracking options
        if ( $bean->aos_invoices_po_purchase_order_1aos_invoices_ida == "") {
            $xml = "    
                    <Options>
                        <Option>
                        <Name>None</Name>
            ...
            $customer_account = new AOS_Invoices();
            $customer_account->retrieve($bean->aos_invoices_po_purchase_order_1aos_invoices_ida);
            // $account_name = $customer_account->billing_account_id;
            $customer_name = new Account();
            $customer_name->retrieve($customer_account->billing_account_id); //<Name>".htmlspecialchars($bean->billing_account)."</Name>
            $xml = "    
                    <Options>
                        <Option>
                        <Name>".htmlspecialchars($customer_name->name)."</Name>
                        </Option>
                    </Options>
                    ";
        }
        ...
        $xero_invoice  = $bean->number;
        $xml = "<PurchaseOrder>";
        if(isset($xero_invoice) && $xero_invoice!="") $xml .=
            "<PurchaseOrderNumber>PO-".htmlspecialchars($xero_invoice)."</PurchaseOrderNumber>";
        ...
        <Contacts>
            <Contact>
            <Name>$bean->billing_account</Name>
            <EmailAddress>$bean_account->email1</EmailAddress>
            </Contact>
        </Contacts>
        ";

        $response = $XeroOAuth->request("POST", $XeroOAuth->url('Contacts', 'core'), array(), $xml_contact);
        
        if ($response['code'] == 200) {
            
        }else {
            $response = $XeroOAuth->request("PUT", $XeroOAuth->url('Contacts', 'core'), array(), $xml_contact);
        }
        ...
        $xml .= "
                    ...
                    <DeliveryDate>".htmlspecialchars($date_for_rebate)."</DeliveryDate>
        ...
        while ($row = $bean->db->fetchByAssoc($result)) {
            if(isset($row) && $row != null ){ 
                ...
                if(isset($product_mapping[$row["product_id"]])){ // <ItemCode>".$product_mapping[$row["product_id"]]."</ItemCode>
                    if($row["product_id"] == "b3c61c17-5bfc-e4f2-1703-5a5ef9506025" && strpos(strtolower($bean->name),"sanden") !== false){
                    ...
                    if ( $bean->aos_invoices_po_purchase_order_1aos_invoices_ida == "") {
                        ...
                        // $account_name = $customer_account->billing_account_id;
                        $customer_name = new Account();
                        $customer_name->retrieve($customer_account->billing_account_id);//<Option>".htmlspecialchars(($account_name?$account_name:$bean->billing_account))."</Option>
                        ...
                        $xml .= "
                            ...
                            ."
                                <Tracking>
                                    <TrackingCategory>
                                        <Name>Customer</Name>
                                        <Option>".htmlspecialchars($customer_name->name)."</Option>
                                    </TrackingCategory>
                                </Tracking>";
                        $xml .= "
                        </LineItem>";
        ...
        $method = 'PUT';
        if( isset($bean->purchase_invoice_xero) && $bean->purchase_invoice_xero!="") {
            //$method = 'POST';
        }
        $response = $XeroOAuth->request($method, $XeroOAuth->url('PurchaseOrders', 'core'), array(), $xml);
        
        if ($XeroOAuth->response['code'] == 200) {

            $invoice = $XeroOAuth->parseResponse($XeroOAuth->response['response'], $XeroOAuth->response['format']);
            $invoice_id = $invoice->PurchaseOrders->PurchaseOrder->PurchaseOrderID;
            
            // Check if exisit xero invoice 
            $bean->purchase_invoice_xero = $invoice_id;
            $bean->save();

            echo "" . count($invoice->PurchaseOrders). " invoice created in this Xero organisation.";
            if (count($invoice->PurchaseOrders)>0) {
                
                downloadPDFFile($templateID, $record);
                $attachmentFile =  file_get_contents(dirname(__FILE__)."/files/invoice-". $record.".pdf");

                $response = $XeroOAuth->request('PUT', $XeroOAuth->url('PurchaseOrders/'.$invoice_id.'/Attachments/invoice-'. $record.'.pdf', 'core'), array(), $attachmentFile, 'file');
                if ($XeroOAuth->response['code'] == 200) {
