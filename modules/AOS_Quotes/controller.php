    ...
    public function action_editview()
    {
        ...
        if (isset($_REQUEST['isDuplicate']) && $_REQUEST['isDuplicate'] == 'true') {
            if ($sugar_config['dbconfig']['db_type'] == 'mssql') {
                $this->bean->number = $this->bean->db->getOne("SELECT MAX(CAST(number as INT))+1 FROM aos_quotes");
            } else {
                $this->bean->number = $this->bean->db->getOne("SELECT MAX(CAST(number as UNSIGNED))+1 FROM aos_quotes");
            }
            //VUT - empty field with Quote type is solar for Duplicate button
            if ($this->bean->quote_type_c == 'quote_type_solar') {
                $this->bean->solargain_quote_number_c = '';
            }
        }
    }
}
