...
// BinhNT
		...
		if($this->solargain_lead_number_c){
			$solar_link = "<a href='https://crm.solargain.com.au/lead/edit/".$this->solargain_lead_number_c."' target='_blank'>L".$this->solargain_lead_number_c."</a><br>";

		}
		if($this->solargain_quote_number_c != ""){
			$solar_link .= "<a href='https://crm.solargain.com.au/quote/edit/".$this->solargain_quote_number_c."' target='_blank'>Q".$this->solargain_quote_number_c."</a>";

		}
		
		$temp_array["SOLARGAIN_LINK"]= $solar_link;

		$temp_array["CONVERT_LIST_LINK"]= "<a href='index.php?module=Leads&action=ConvertLead&record=".$this->id."' target='_blank'>Convert</a>";
		
		$custom_email_link = "";
		if($this->email1){
			$search_link = "<a target='_blank' href='https://mail.google.com/#search/".urlencode($this->email1)."'>GMSearch</a>";
			$send_gmail_link = "<a target='_blank' href='https://mail.google.com/?view=cm&fs=1&tf=1&to=".urlencode($this->email1)."&su=".$this->name."'>SendGmail</a>";
			$send_pe_email = '<a class="email-link" href="javascript:void(0);" onclick="$(document).openComposeViewModal(this);" data-module="Leads" data-record-id="'.$this->id.'" data-module-name="'.$this->name.'" data-email-address="'.$this->email1.'">'.$this->email1.'</a>';
    ...
    */
    ...
    public function save($check_notify = false)
    {
		...
		if (empty($this->id) || $this->new_with_id || empty($this->number)){
			if($sugar_config['dbconfig']['db_type'] == 'mssql'){
				$this->number = $this->db->getOne("SELECT MAX(CAST(number as INT))+1 FROM leads");
			} else {
				$this->number = $this->db->getOne("SELECT MAX(CAST(number as UNSIGNED))+1 FROM leads");
			}
		}
		//end dung code
		if(empty($this->status))
    ...
    public function get_pe_smsmanager()
    {
        $return_array['select']='SELECT pe_smsmanager.id ';
        $return_array['from']='FROM pe_smsmanager  ';
        $return_array['where']="pe_smsmanager.deleted=0 ";
        $return_array['join'] = " INNER JOIN pe_smsmanager_leads_c ON pe_smsmanager.id=pe_smsmanager_leads_c.pe_smsmanager_leadspe_smsmanager_ida AND pe_smsmanager_leads_c.pe_smsmanager_leadsleads_idb='$this->id' AND pe_smsmanager_leads_c.deleted=0";
        $return_array['join_tables'][0] = '';
        return $return_array;
