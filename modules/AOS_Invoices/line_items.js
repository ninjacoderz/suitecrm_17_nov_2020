...
function clearPreviousTextBox(productLine){
  $("#product_serial_number"+productLine).val("");
}
...
function insertLineItems(product,group){
  ...
  for(var p in product){
    if(document.getElementById(type + p + ln) !== null){
      if(product[p] !== '' && isNumeric(product[p]) && p != 'vat'  && p != 'product_id' && p != 'name' && p != "part_number" && p != "serial_number" && p != 'description'){
...
function insertProductLine(tableid, groupid) {
 ...
 // BinhNT Code here
 a.innerHTML = "<span class='handle'> + </span><input name='product_number["+prodln+"]' type='hidden' value='"+( prodln + 1 )+"'><input type='text' name='product_product_qty[" + prodln + "]' id='product_product_qty" + prodln + "'  value='' title='' tabindex='116' onblur='Quantity_format2Number(" + prodln + ");calculateLine(" + prodln + ",\"product_\");' class='product_qty'>";
  ...
  h1.innerHTML = "<span style='vertical-align: top;' class='product_item_description_label'>" + SUGAR.language.get(module_sugar_grp1, 'LBL_SERIAL_NUMBER') + " :&nbsp;&nbsp;</span>";
  h1.innerHTML += "<input type='text' name='product_serial_number[" + prodln + "]' id='product_serial_number" + prodln + "' maxlength='1000' value='' title='' tabindex='116' class='product_serial_number'>";
  h1.innerHTML += "<button title='" + SUGAR.language.get('app_strings', 'LBL_SELECT_BUTTON_TITLE') + "' accessKey='" + SUGAR.language.get('app_strings', 'LBL_SELECT_BUTTON_KEY') + "' type='button' tabindex='116' class='button product_serial_number_button' value='" + SUGAR.language.get('app_strings', 'LBL_SELECT_BUTTON_LABEL') + "' name='btn1' onclick='openStockPopup(" + prodln + ");'><span class=\"suitepicon suitepicon-action-select\"></span></button>";
  h1.innerHTML += '<button type="button" name="btn_clr_serials_number" id="btn_clr_serials_number" tabindex="0" title="{$MOD.LBL_DASHLET_CLEAR_REPORT}"  class="button lastChild"\
                            onclick="return clearPreviousTextBox('+prodln+');"  value="{$MOD.LBL_DASHLET_CLEAR_REPORT}" >\
                            <span class="suitepicon suitepicon-action-clear"></span>\
                        </button>';
  h1.style.padding = '0';
  // BinhNT End more info
  ...
  h1.style.padding = '0 0 32px 0';//thienpb fix
  ...
  i.style.padding = '0 0 32px 0';//thienpb fix
...
//thienpb code
function openStockPopup(ln){

  lineno=ln;
  var popupRequestData = {
    "call_back_function" : "setStockReturn",
    "form_name" : "EditView",
    "field_to_name_array" : {
      "serial_number" : "product_serial_number" + ln,
    }
  };
  var query = '&part_number_advanced=' + $("#product_part_number"+ln).val();
  //query += '&invoice_c_advanced=NULL';
  open_stock_popup('pe_stock_items', 800, 850, query , true, true, popupRequestData, "multiselect");

}

function open_stock_popup(module_name, width, height, initial_filter, close_popup, hide_clear_button, popup_request_data, popup_mode, create, metadata) {
...
//thienpb code
function setStockReturn(popupReplyData){
...
function calculateTotal(key)
{
  ...
  // if(typeof populatePaymentAmout_change_total == 'function') {
  //   try {
  //     populatePaymentAmout_change_total()
  //   } catch (error) {
  //     console.log(error);
  //   }
  // }
