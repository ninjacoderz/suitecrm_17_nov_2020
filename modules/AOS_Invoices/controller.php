    ...
    public function action_editview()
    {
        ...
        if (isset($_REQUEST['isDuplicate']) && $_REQUEST['isDuplicate'] == 'true') {
            if ($sugar_config['dbconfig']['db_type'] == 'mssql') {
                $this->bean->number = $this->bean->db->getOne("SELECT MAX(CAST(number as INT))+1 FROM aos_invoices");
            } else {
                $this->bean->number = $this->bean->db->getOne("SELECT MAX(CAST(number as UNSIGNED))+1 FROM aos_invoices");
            }
            $array_fields_not_duplicate = ['payments_c','xero_invoice_c','xero_stc_rebate_invoice_c','xero_shw_rebate_invoice_c','xero_veec_rebate_invoice_c','installation_pictures_c'];
            foreach ($array_fields_not_duplicate as  $value) {
                $this->bean->{$value} = '';
            }
        }
    }   
