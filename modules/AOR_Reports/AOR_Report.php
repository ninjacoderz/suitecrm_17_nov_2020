...
class AOR_Report extends Basic
{
    ...
    public function build_report_chart($chartIds = null, $chartType = self::CHART_TYPE_PCHART)
    {
        ...
        foreach ($linkedCharts as $chart) {
            //thienpb change check $chartIds
            if (count($chartIds) > 0 && !in_array($chart->id, $chartIds)) {
                continue;
            }
        ...
        while ($fieldCount && $row = $this->db->fetchByAssoc($result)) {
            ...
            foreach ($fields as $name => $att) {
                if ($att['display']) {
                    ...
                    if ($att['link'] && $links) {
                        //dung code - check open new tab link solargain
                        if( $att['field']  == 'solargain_quote_number_c'){
                            $html .= "<a target='_blank' href='https://crm.solargain.com.au/quote/edit/" . $row[$name] . "'>";  
                        }elseif($att['field'] == 'solargain_lead_number_c'){
                            $html .= "<a target='_blank' href='https://crm.solargain.com.au/lead/edit/" . $row[$name] . "'>";  
                        }elseif($att['field'] == 'solargain_tesla_quote_number_c'){
                            $html .= "<a target='_blank' href='https://crm.solargain.com.au/quote/edit/" . $row[$name] . "'>";  
                        }
                        else {
                            $html .= "<a target='_blank' href='" . $sugar_config['site_url'] . "/index.php?module=" . $att['module'] . "&action=DetailView&record=" . $row[$att['alias'] . '_id'] . "'>";
                        }                    
                    }//tu-code show link-the-tank-and-hp-serials-to-the-stock-items-for-sanden-serial-number-report
                    elseif($att['field'] == "sanden_tank_serial_c" || $att['field'] == "sanden_hp_serial_c"){
                        $number_serial = str_replace(" ","",$row[$name]);
                        $id = explode(',',strip_tags($number_serial));
                            for ( $stock = 0 ; $stock < count($id); $stock++ ){
                                
                                $db = DBManagerFactory::getInstance();
                                $sql = "SELECT id,parent_id FROM pe_stock_items WHERE serial_number ='$id[$stock]' AND deleted = 0";
                                $ret = $db->query($sql);
                                    if($ret->num_rows > 0){
                                        if( $tvat = $ret ->fetch_assoc()){
                                            $record = $tvat['id'];
                                            if($record != '' && $record != NULL){
                                                $pe_warehouse_log = new pe_warehouse_log();
                                                $pe_warehouse_log ->retrieve($tvat['parent_id']);
                                                $id_whlog =  $pe_warehouse_log->id;
                                                if(  $id_whlog != Null) {
                                                $html .= "<a target='_blank' href='https://suitecrm.pure-electric.com.au/index.php?module=pe_stock_items&action=DetailView&record=$record'>$id[$stock]</a>";  
                                                }else {
                                                $html .= "<span title='No Warehouse Log'>$id[$stock]</span>";
                                                }
                                            }
                                        }
                                        if ( count($id) > 1) {
                                        ...
                                        $html .= "<span title='Not Link'>$id[$stock]</span>";
                                        if ( count($id) > 1) {
                                            $html .= " , ";
                                        }
                                    }
			...
			// merge code
                    if($att['field'] != "sanden_tank_serial_c" && $att['field'] != "sanden_hp_serial_c"){
                        $currency_id = isset($row[$att['alias'] . '_currency_id']) ? $row[$att['alias'] . '_currency_id'] : '';
                    ...
                    if ($att['function'] == 'COUNT' || !empty($att['format'])) {
                        ...
                        if ($att['link'] && $links) {
                            $html .= "</a>";
                            if(array_search($name,array_keys($fields)) == 0){
                                $html .= "<a target='_blank' href='" . $sugar_config['site_url'] . "/index.php?module=" . $att['module'] . "&action=EditView&record=" . $row[$att['alias'] . '_id'] . "'>&nbsp;&nbsp;[E]</a>";
                            }
                        }
    ...
    {
        global $beanList, $app_list_strings, $sugar_config, $timedate,$current_user;
        ...
        $aor_sql_operator_list['Does_Not_Contains'] = 'NOT LIKE';//custom code-- add new type condition report
                ...
                if (!empty($path[0]) && $path[0] != $module->module_dir) {
                    foreach ($path as $rel) {
                        ...
                        if (!(ACLController::checkAccess($new_condition_module->module_name, 'list', true))) {
                            //return false; // BinhNT We allow all user from here
                        }
                        $oldAlias = $table_alias;
                        $table_alias = $table_alias . ":" . $rel;
                        $query = $this->build_report_query_join(
                    ...
                    switch ($condition->value_type) {
                        ...
                        case 'Date':
                            ...
                            if ($params[0] == 'now') {
                                ...
                                //thienpb code
                                if ($params[0] == 'this_month') {
                                    $value = 'DATE_ADD(Curdate(), interval -DAY(Curdate())+1 DAY)';
                                }else if ($params[0] == 'this_year') {
                                    $value = 'DATE(Curdate()-interval DAYOFYEAR(Curdate()-1) DAY)';
