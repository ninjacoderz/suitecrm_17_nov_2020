...
EOF;
    ...
    private function getRGraphGroupedBarChart($chartDataValues, $chartLabelValues, $chartTooltips, $chartName, $chartId, $chartHeight = 400, $chartWidth = 400, $grouped = false)
    {
        ...
        //thienpb custom
        if($chartName == 'Sales Chart' || $chartName == 'Purchases Chart'){
            $this->colours = '["green","red","blue"]';
            $hint = json_decode($chartTooltips);
            $color_hint = json_decode($this->colours);

            $html = '<div style="margin-left: 50%;"><span style="width:10px;height:10px;background-color:'.$color_hint[0].';display:inline-block;margin-left:10px;"></span> '.date('M Y',strtotime(explode(" (",$hint[0])[0]));
            $html .= '<span style="width:10px;height:10px;background-color:'.$color_hint[1].';display:inline-block;margin-left:10px"></span> '.date('M Y',strtotime(explode(" (",$hint[1])[0]));
            $html .= '<span style="width:10px;height:10px;background-color:'.$color_hint[2].';display:inline-block;margin-left:10px"></span> '.date('M Y',strtotime(explode(" (",$hint[2])[0])).'</div>';
        }else{
            $html = '';
        }
        $html .= "<canvas id='$chartId' width='$chartWidth' height='$chartHeight' class='resizableCanvas'></canvas>";
