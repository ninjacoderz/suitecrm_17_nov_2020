...
//dung code - fix  task paul - not result when type value in payment field group
function updatePaymentInfo_Report(){
    ...
    return encodeURIComponent(JSON.stringify(listPaymentObject));
}
...
$(function () {
    ...
    $('#multipayment tr input[name="payment_date"]').each(function(){
        YAHOO.util.Event.addListener($(this).attr('id'), "change", function(){
            $('#multipayment').siblings('input').val(updatePaymentInfo_Report());
            $('#multipayment').siblings('input').attr('id',$('#multipayment').siblings('input').attr('name'));// for Detail View click button Upload - because it get value by ID but it not have attribute id 
            console.log($('#multipayment').siblings('input').val());
        });
    });
    $(document).on('focusout',"input[name='payment_amount']", function(){
        $('#multipayment').siblings('input').val(updatePaymentInfo_Report());
        $('#multipayment').siblings('input').attr('id',$('#multipayment').siblings('input').attr('name'));// for Detail View click button Upload - because it get value by ID but it not have attribute id 
        addTextRow();
        console.log($('#multipayment').siblings('input').val());
