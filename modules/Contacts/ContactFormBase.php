...
EOQ;
    ...
    {
        global $theme, $current_user, $sugar_config;
        ...
        if (!empty($_POST[$prefix.'new_reports_to_id'])) {
            ...
            //custom code -- auto inrease number
            if (empty($focus->id)) {
                if ($sugar_config['dbconfig']['db_type'] == 'mssql') {
                    $focus->number = (int)$focus->db->getOne("SELECT MAX(CAST(number as INT))+1 FROM contacts");
                } else {
                    $focus->number = (int)$focus->db->getOne("SELECT MAX(CAST(number as UNSIGNED))+1 FROM contacts");
                }
            }
