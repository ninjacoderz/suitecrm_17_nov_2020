...
$time_file = strtotime(date("F d Y H:i:s.", filemtime($filename)))+43200;
$date =  strtotime(date('F d Y H:i:s.', time()));
if( $time_file <= $date){
    $folder_info = shell_exec('df -h');
    // $folder_info ="Filesystem      Size  Used Avail Use% Mounted on
    //             /dev/xvda2       94G   44G   28G  71% /
    ...
    $checknumber = (string)preg_replace("/\r\n|\n/",'<br>',strip_tags($folder_info)); 
    // $checknumber = strip_tags($folder_info);
    $checknumber = str_replace(' ','',$checknumber);
    $find_number = '/G(.*?)devtmpfs/s';
    $data_return = preg_match($find_number,$checknumber,$matches);
    $checknumber = str_replace('%/<br>','',$matches[1]);
    $checknumber = intval(substr($checknumber,-2));
    $folder_info = (string)preg_replace("/\r\n|\n/",'<br>',strip_tags($folder_info)); 
    if( !isset($_COOKIE['checksend'])){
        if( $checknumber >= 66 &&  $checknumber < 85   ){
            send_mail($checknumber);
            $checksend = 3;
            setcookie('checksend',$checksend,time() + (86400 * 30), "/");
        }elseif ($checknumber >= 85 &&  $checknumber < 95 ){
            send_mail($checknumber);
            $checksend = 3;
            setcookie('checksend',$checksend,time() + (86400 * 30), "/");
        }         
    ...
    if( isset($_COOKIE['checksend'])){
        if( $checknumber >= 85 &&  $checknumber < 95 &&  $_COOKIE['checksend'] < 5){
            send_mail($checknumber);
            $checksend = $_COOKIE['checksend'] + 1;
            setcookie('checksend',$checksend,time() + (86400 * 30), "/");
        }elseif ( $checknumber >= 66 &&  $checknumber < 85 &&  $_COOKIE['checksend'] == 5){
            send_mail($checknumber);
            $checksend = 3;
            setcookie('checksend',$checksend,time() + (86400 * 30), "/");
        }
    ...
    $folder_info = file_get_contents('severinfo.txt', true);
    $folder_info = (string)preg_replace("/\r\n|\n/",'<br>',strip_tags($folder_info)); 
}
function send_mail($checknumber){
        ...
        $body = 'Attention, the Pure Electric server is now '.$checknumber.'% full!';
        $mail->Body = $body ;
...
$data = file_get_contents("https://suitecrm.com/download/");
//$pattern = '/SuiteCRM ([0-9].[0-9]*.[0-9]*)/';
$pattern = '/style="font-size:25px;line-height:30px;">SuiteCRM 7(.*?)<\/h2><\/div><div class="fusion-clearfix">/s';
$data_return = preg_match($pattern,$data,$match);
?>
<style>
    .row-info-system {
...
<div class='row row-info-system'>
    <div class="col-md-6 item-row">
        <h3>SERVER INFO</h3>
        <!-- <pre><br>Filesystem      Size  Used Avail Use% Mounted on<br>/dev/xvda2       94G   61G   34G  65% /<br>devtmpfs        471M     0  471M   0% /dev<br>tmpfs           495M     0  495M   0% /dev/shm<br>tmpfs           495M     0  495M   0% /sys/fs/cgroup<br>tmpfs           495M   51M  445M  11% /run<br>tmpfs            99M     0   99M   0% /run/user/0<br>tmpfs            99M     0   99M   0% /run/user/1000<br></pre> -->
        <pre><?php echo $folder_info;?></pre>
    </div>
    <div class="col-md-6 item-row">
        <h3>SuiteCRM Version</h3>
        <h3>SuiteCRM <?php echo $suitecrm_version ?> installed</h3>
        <h3>Lastest Version <a href="https://suitecrm.com/download/"> 7<?= $match[1] ?></a>
    </div>
</div>