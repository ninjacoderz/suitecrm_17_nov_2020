...
class Call extends SugarBean
{
    ...
    public function __construct()
    {
        ...
        // custom code -- override seperate type minutes in duration time 
        $this->minutes_values  = array (
            0 => '00',
            05 => '05',
            10 => '10',
            15 => '15',
    ...
    public function save($check_notify = false)
    {
		...
		if (empty($this->id) || $this->new_with_id){
			if($sugar_config['dbconfig']['db_type'] == 'mssql'){
				$this->number = $this->db->getOne("SELECT MAX(CAST(COALESCE(number,0) as INT))+1 FROM calls");
			} else {
				$this->number = $this->db->getOne("SELECT MAX(CAST(COALESCE(number,0) as UNSIGNED))+1 FROM calls");
			}
        }
        /**Save unique number */
    ...
    */
    ...
    public function get_list_view_data()
    {
        ...
        //custom get email address from account and contact
        $email_address= '';
        $module_name = '';
        $record_id = '';
        $name = '';
        ...
        if($this->parent_type == 'Accounts') {
            $account_bean = BeanFactory::getBean("Accounts", $this->parent_id);
            $email_address= $account_bean->email1;
            $module_name = 'Accounts';
            $record_id = $account_bean->id;
            $name = $account_bean->name;
        }
       
        if(empty($email_address)){
            $contact_bean = BeanFactory::getBean("Contacts", $this->contact_id);
            $email_address= $contact_bean->email1;
            $module_name = 'Contacts';
            $record_id = $contact_bean->id;
            $name = $contact_bean->first_name . ' ' .$contact_bean->last_name;
        }
        $email_address_in_call = $this->createEmailLink($module_name, $record_id, $name, $email_address);
        $call_fields['EMAIL_ADDRESS_IN_CALL'] =  $email_address_in_call;
        return $call_fields;
    }
    ...
    public function createEmailLink($module_name, $record_id, $name, $addr)
    {
        global $current_user;
        $html_copy_email = '';
        if($addr){   
                $html_copy_email = <<<HTML
                <a class="copy-email-link" data-module-name="{$module_name}" data-email-address="{$addr}"
                 ...
                 <span class="tooltiptext" style="display:none;width:200px;background:#94a6b5;color:#fff;text-align: center;border-radius: 6px;padding: 5px 0; position: absolute;z-index: 1;">Copied {$addr}</span>
                 </a>
               HTML;
        }
