<?php
    ...
    class QOutfittersLicense
    {
        ...
        public static function isValid($thisModule=null,$user_id=null,$force_validation=false)
        {
            ...
            $validated = QOutfittersLicense::doValidate($thisModule);
        ...
        public static function validate()
        {
            ...
            $validated = QOutfittersLicense::doValidate($currentModule,$_REQUEST['key']);
...
*/                
         ...
         */
        public static function doValidate($thisModule,$key=null,$users_count=null)
        {
            global $sugar_config;
            ...
            {
                $GLOBALS['log']->fatal('QOutfittersLicense::doValidate() failed: '.print_r($response,true));
            }
        ...
        public static function change()
        {
            ...
            {
                header('HTTP/1.1 400 Bad Request');
                $GLOBALS['log']->fatal('QOutfittersLicense::change() failed: '.print_r($response,true));
            } 
            else 
            {
                require_once('modules/Administration/Administration.php');
        ...
        public static function add()
        {
            ...
            $response = QOutfittersLicense::doValidate($currentModule,null,count($_REQUEST['licensed_users']));
            ...
            foreach($_REQUEST['licensed_users'] as $licensed_user) {
				...
				if ($sugar_version >= '6.5'){
					...
					$query = "INSERT INTO so_users (id,shortname,user_id,deleted) VALUES ( '". $db->quote($data['id']). "','" . $db->quote($data['shortname']). "','" . $db->quote($data['user_id']). "', 0)";
					$db->query($query);
				}
            }
    ...
    class SugarOutfitters_API
    {
        private function get_default_payload($module,$custom_data = array())
        {
            ...
            if (empty($custom_data['key'])){
                $key = QOutfittersLicense::getKey($module);
            ...
            $data['sugar_fork'] = empty($sugar_config['suitecrm_version']) ? $not_set_value : $sugar_config['suitecrm_version'];
