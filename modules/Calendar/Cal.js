...
CAL.change_activity_type = function(mod_name) {
    ...
    $('#calendarContainer .fc-content-skeleton .fc-event-container .fc-content .fc-title').each(function(){
        var  meeting_delivery_c = $(this).html().substr(0,7);
           if( meeting_delivery_c == "Dispatc" ){
               $(this).parent().parent().css('background-color','rgb(175, 144, 5)');
           }else if( meeting_delivery_c == "Arrival" ){
               $(this).parent().parent().css('background-color','rgb(156, 16, 126)')
           }
    }) 
}
    ...
    function constructCalendar(user_id, all_events) {
        ...
        $('#calendar' + user_id).fullCalendar({
            ...
            eventDrop: function(event, delta, revertFunc) {
                var date_star = event.start.format("DD/MM/YYYY HH:mm");
                var meeting_delivery =event.title.substr(0,7);
                var record =  event.id;
                ...
                if( meeting_delivery == "Dispatc" ){
                    $.ajax({
                        url: "index.php?entryPoint=createMettingWHLog&record_id=" + record +"&dispatch_date="+date_star+"&update_met=update_move_to_WH_log",
                        type: 'GET',
                        async: false,
                    ...
                    $.ajax({
                        url: "index.php?entryPoint=createMettingWHLog&record_id=" + record +"&arrival_date="+date_star+"&update_met=update_move_to_WH_log",
                        type: 'GET',
                        async: false,
                        success: function(data){
                            console.log('update seccess!')
