...
class Email extends Basic
{
    ...
    public function handleMultipleFileAttachments()
    {
        ...
        if (isset($_REQUEST['template_attachment']) && !empty($_REQUEST['template_attachment'])) {
            ...
            foreach ($_REQUEST['template_attachment'] as $noteId) {
                ...
                //fix bug - duplicate notes when save draft
                if (isset($_REQUEST['action']) && !empty($_REQUEST['action']) && $_REQUEST['action'] == 'SaveDraft') {
                   continue;
                }
                $noteTemplate = BeanFactory::newBean('Notes');
                $noteTemplate->retrieve($noteId);
            ...
            $this->attachments = array_merge($this->attachments, $noteArray);
            // Handle attachment for Lead
            // 1. get the lead and with list of file attachment
            // 2. Create Node 
            // 3. Merge array
            ...
            if($_REQUEST["parent_type"]== "Leads"){
                $lead_bean = new Lead();
                $noteArray = array();
                $lead_bean = $lead_bean->retrieve($_REQUEST['parent_id']);
                //if(isset($lead_bean->file_attachment_c) && $lead_bean->file_attachment_c != ""){
                    //$lead_attachments = json_decode(htmlspecialchars_decode($lead_bean->file_attachment_c));
                    $lead_file_attachmens = array_unique($_REQUEST["lead_file_attachments"]);
        ...
        $_FILES["email_attachment"]["size"] = array_unique($_FILES["email_attachment"]["size"]);
    ...
    public function fill_in_additional_detail_fields()
    {
        ...
        if ($row != null) {
            ...
            $this->parent_name = $contact->full_name; // BinhNT Quick Fix
            ...
            if($this->parent_type == "Accounts"){ //thienpb fix
                $sql_query = 'SELECT accounts.name FROM accounts WHERE accounts.id = "'.$this->parent_id.'"';
                $result = $this->db->query($sql_query, true, " Error filling in additional detail fields: ");
                $row = $this->db->fetchByAssoc($result);
                if($row != null){
            ...
            if($this->parent_type == "Leads"){ //thienpb fix
                $sql_query = 'SELECT leads.first_name,leads.last_name FROM leads WHERE leads.id = "'.$this->parent_id.'"';
                $result = $this->db->query($sql_query, true, " Error filling in additional detail fields: ");
                $row = $this->db->fetchByAssoc($result);
                if($row != null){
