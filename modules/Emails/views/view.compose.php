...
class EmailsViewCompose extends ViewEdit
{
	...
	// BinhNT
        ...
        if (isset($lead_id) && $lead_id != ""){
            ...
            if(isset($lead->file_attachment_c) && $lead->file_attachment_c != ""){
                $lead_attachments = json_decode(htmlspecialchars_decode($lead->file_attachment_c));
                if(count($lead_attachments)>0){
                    $this->ev->ss->assign('LEAD_ATTACHMENTS', $lead_attachments);
                }
            }
        }
    // Thienpb fix
        $invoice_id =  $_REQUEST['sendGeo_invoice_id']; 
        $this->ev->ss->assign('sendGeo_invoice_id',$invoice_id);  
