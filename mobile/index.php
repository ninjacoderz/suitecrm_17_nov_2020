...
<html>
    <head>
        ...
        <link rel="stylesheet" href="css/quickcrm6.0.1.css" />
...
function cachedFile (url,callback) {
	var cached=localStorage.getItem('file-'+url);
	if(cached){
		callback(cached);
		return;
	}
	...
	return $.ajax(AjaxOptions).success(function(res){
			try {
				if (QCRM.QBasicAuth){
					res = atob(res.response);
				}
				callback(res);
				try{
				...
				try{
					localStorage.setItem('file-'+url,res);
				}
				catch(err){}
			}
			catch(err){
...
//	var conf_script=ServerAddress+'/index.php?entryPoint=QuickCRMgetConfig'+(trial!==false?'&trial=yes':'');
	...
	$.getScript(conf_script+"&param=sugar_config")
		.done(function(script, textStatus) {
			...
			$.cachedScript("js/quickcrm-web-6.1.min.js").done(function(script, textStatus) {
				...
				if (mobile_edition!=='CE'){
					...
					if (!(QCRM.sugaroutfitters || QCRM.usersTable)) cachedScript(conf_script+"&param=users",{error:function(){err=1;}});
    ...
    <body>
        ...
        <div id="HomePage" data-role="page" data-theme="c" data-title="Home" class="ui-responsive-panel">
            <div data-role="panel" data-theme="a" id="HomePanel" data-display="reveal" data-dismissible="false">
				...
				<br>
				<div>
					<ul id="CustomPanelDiv" data-role="listview" data-filter="false" style="display:none;list-style: none;margin: 0;padding: 0;">
					</ul>
        ...
        <div id="DebugLogPage" data-role="page" data-theme="c">
            <div data-role="header" data-theme="a">
				<a href="#" data-role="button" data-rel="back"  data-icon="arrow-l" data-iconpos="notext" data-iconshadow="false" class="ui-nodisc-icon"></a>
                <h1>Debug log</h1>
           </div>
            <div role="main" class="ui-content">
					<div id="DebugLog">
