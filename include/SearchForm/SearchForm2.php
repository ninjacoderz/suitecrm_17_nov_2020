            ...
            if (!empty($parms['enable_range_search']) && empty($type)) {
                if (preg_match('/^start_range_(.*?)$/', $field, $match)) {
                    ...
                    if (preg_match('/^range_(.*?)$/', $field, $match) && isset($this->searchFields[$field]['value'])) {
                        ...
                        // custom search ranger by number days
                        if (preg_match('/^start_days_range_(.*?)$/', $field, $match)) {
                            $real_field = $match[1];
                            $start_field = 'start_days_range_' . $real_field;
                            $end_field = 'end_days_range_' . $real_field;
        
                            if (isset($this->searchFields[$start_field]['value']) && isset($this->searchFields[$end_field]['value'])) {
                                $this->searchFields[$real_field]['value'] = $this->searchFields[$start_field]['value'] . '<>' . $this->searchFields[$end_field]['value'];
                                $this->searchFields[$real_field]['operator'] = 'between_days';
                                $parms['value'] = $this->searchFields[$real_field]['value'];
                                $parms['operator'] = 'between_days';
                    ...
                    foreach ($parms['db_field'] as $db_field) {
                                                ...
                                                if ($operator == 'greater_than_equals') {
                                                    ...
                                                    if ($operator == 'less_than_equals') {
                                                        ...
                                                        if ($operator == 'between_days') {
                                                                ...
                                                                $number_days_start =  (int)$field_value[0];
                                                                $number_days_end =  (int)$field_value[1];
                                
                                                                $date_condition_days_start ="'" . date('Y-m-d', (time()-24*$number_days_start*60*60)) ."'";
                                                                $date_condition_days_end = "'" . date('Y-m-d', (time()+24* $number_days_end*60*60)) ."'";
                                                                // FG - bug45287 - Note "start" and "end" are the correct interval at GMT timezone
                                                                $field_value = array($date_condition_days_start, $date_condition_days_end);
                        ...
                        switch (strtolower($operator)) {
                            ...
                            case 'between':
                                ...
                                break;
                            //  custom between ranger from -7 days and +7 days
                            case 'between_last_and_next_7_days':
                                $date_condition_7days_start ="'" . date('Y-m-d', (time()-24*7*60*60)) ."'";
                                $date_condition_7days_end = "'" . date('Y-m-d', (time()+24*7*60*60)) ."'";
                                $where .= "($db_field >= {$date_condition_7days_start} AND $db_field <= {$date_condition_7days_end})";
                                break;
                            // custom search ranger by number days
                            case 'between_days':
                                if (!is_array($field_value)) {
                                    ...
                                    $date_condition_days_start = $field_value[0];
                                    $date_condition_days_end =  $field_value[1];
                                }
                              
                                $where .= "($db_field >= {$date_condition_days_start} AND $db_field <= {$date_condition_days_end})";
                                break;
                            case 'date_not_equal':
                                if (!is_array($field_value)) {
                                    $field_value = explode('<>', $field_value);
