...
<div id="{$id}_range_div" style="{if preg_match('/^\[/', $smarty.request.{{$id_range}})  || $starting_choice == 'between' || $starting_choice == 'between_days'}display:none{else}display:''{/if};">
...
{*custom code - add search ranger for Calls*} 

{{if $module == 'Calls'}}
    <div id="{$id}_between_days_range_div" style="{if $starting_choice=='between_days'}display:'';{else}display:none;{/if}">
        {assign var=date_value value={{sugarvar key='value' string=true}} }
        From
        <input autocomplete="off" type="text" name="start_days_range_{$id}" id="start_days_range_{$id}" value='{$smarty.request.{{$id_range_start_days}} }' title='{{$vardef.help}}' {{$displayParams.field}} tabindex='{{$tabindex}}' size="11" class="dateRangeInput">
        previous days to
        {assign var=date_value value={{sugarvar key='value' string=true}} }
        <input autocomplete="off" type="text" name="end_days_range_{$id}" id="end_days_range_{$id}" value='{$smarty.request.{{$id_range_end_days}} }' title='{{$vardef.help}}' {{$displayParams.field}} tabindex='{{$tabindex}}' size="11" class="dateRangeInput" maxlength="10">
        days after
    </div>
...
{{else}}
    <div id="{$id}_between_days_range_div" style="{if $starting_choice=='between_days'}display:'';{else}display:none;{/if}">
        {assign var=date_value value={{sugarvar key='value' string=true}} }
        From
        <input autocomplete="off" type="text" name="start_days_range_{$id}" id="start_days_range_{$id}" value='' title='{{$vardef.help}}' {{$displayParams.field}} tabindex='{{$tabindex}}' size="11" class="dateRangeInput">
        previous days to
        {assign var=date_value value={{sugarvar key='value' string=true}} }
        <input autocomplete="off" type="text" name="end_days_range_{$id}" id="end_days_range_{$id}" value='' title='{{$vardef.help}}' {{$displayParams.field}} tabindex='{{$tabindex}}' size="11" class="dateRangeInput" maxlength="10">
        days after
    </div>
{{/if}}
...
{ldelim}
  if(val == 'between') {ldelim}
     ...
     document.getElementById("{$id}_between_days_range_div").style.display = 'none';
  {rdelim} else if (val == 'between_days' ){ldelim}
     document.getElementById("range_{$id}").value = '';  
     document.getElementById("{$id}_range_div").style.display = 'none';
     document.getElementById("{$id}_between_range_div").style.display = 'none'; 
     document.getElementById("{$id}_between_days_range_div").style.display = '';
