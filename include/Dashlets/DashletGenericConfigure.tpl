...
<div>
    <form action='index.php' id='configure_{$id}' method='post'
          onSubmit='SUGAR.mySugar.setChooser(); return SUGAR.dashlets.postForm("configure_{$id}", SUGAR.mySugar.uncoverPage);' style="max-height:650px;overflow:auto;">
                ...
                <td colspan='4' align='right'>
                    ...
                    {if $showClearButton}
                        <input type='submit' class='button' value='{$strings.clear}'
                               onclick='SUGAR.searchForm.clear_form(this.form,["dashletTitle","displayRows","autoRefresh"]);$(".chosen-select").val("").trigger("chosen:updated");return false;'>
                    {/if}
                </td>
            </tr>
        </table>
