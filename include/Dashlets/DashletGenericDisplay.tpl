                ...
                <tr>
                    ...
                    <td align='right' nowrap='nowrap'>
						...
						{if $pageData.bean.moduleDir == 'Opportunities'}
								<button title='{$navStrings.start}' class='button primary' id="op_btn_today" onclick='return SUGAR.mySugar.retrieveDashlet("{$dashletId}", "{$pageData.urls.jump_today}&sortOrder_closedate=1", false, false, true, $(this).closest("div[id^=pageNum_][id$=_div]").parent().parent())'>
									TODAY
								</button>
								<button title='{$navStrings.start}' class='button primary' id="op_btn_reset" onclick='return SUGAR.mySugar.retrieveDashlet("{$dashletId}", "{$pageData.urls.reset_today}", false, false, true, $(this).closest("div[id^=pageNum_][id$=_div]").parent().parent())'>
									RESET
								</button>
						{/if}
						...
						{if $pageData.bean.moduleDir == 'AOS_Invoices'}
                                ...
                                <button title='Filter Mathven' class='button primary' style='padding: 5px;background-color: #f08377;' id="op_btn_methven" onclick='return SUGAR.mySugar.retrieveDashlet("{$dashletId}", "{$pageData.urls.product_type}&product_type=methven", false, false, true, $(this).closest("div[id^=pageNum_][id$=_div]").parent().parent())'>
                                    METHVEN
                                </button>
								<button title='Filter Solar' class='button primary' style='padding: 5px;background-color: #f08377;' id="op_btn_solar" onclick='return SUGAR.mySugar.retrieveDashlet("{$dashletId}", "{$pageData.urls.product_type}&product_type=solar", false, false, true, $(this).closest("div[id^=pageNum_][id$=_div]").parent().parent())'>
                                    SOLAR PV
                                </button>
								<button title='Filter Daikin_us7' class='button primary' style='padding: 5px;background-color: #f08377;' id="op_btn_daikin_us7" onclick='return SUGAR.mySugar.retrieveDashlet("{$dashletId}", "{$pageData.urls.product_type}&product_type=daikin_us7", false, false, true, $(this).closest("div[id^=pageNum_][id$=_div]").parent().parent())'>
                                    DAIKIN US7
                                </button>
								<button title='Filter Daikin_nexura' class='button primary' style='padding: 5px;background-color: #f08377;' id="op_btn_daikin_nexura" onclick='return SUGAR.mySugar.retrieveDashlet("{$dashletId}", "{$pageData.urls.product_type}&product_type=daikin_nexura", false, false, true, $(this).closest("div[id^=pageNum_][id$=_div]").parent().parent())'>
                                    DAIKIN NEXURA
                                </button>
								<button title='{$navStrings.start}' class='button primary' style='padding: 5px;background-color: #f08377;' id="op_btn_reset" onclick='return SUGAR.mySugar.retrieveDashlet("{$dashletId}", "{$pageData.urls.reset_product_type}&product_type=reset", false, false, true, $(this).closest("div[id^=pageNum_][id$=_div]").parent().parent())'>
									RESET
								</button>
                    	{/if}
	...
	{foreach name=rowIteration from=$data key=id item=rowData}
		...
		<tr height='20' class='{$_rowColor}S1'>
			...
			{foreach from=$displayColumns key=col item=params}
			    {strip}
				{*
				BinhNT Code HERE inline editing 
				*}
				<td scope='row' align='{$params.align|default:'left'}' valign="top" field="{$col|lower}"  type="{$displayColumns.$col.type}" class="{$hide} {if $inline_edit && ($displayColumns.$col.inline_edit == 1 || !isset($displayColumns.$col.inline_edit))}inlineEdit{/if}{if ($params.type == 'teamset')}nowrap{/if}" current-module="{$pageData.bean.moduleDir}" record-id="{$rowData.ID}">
					{if $col == 'NAME' || $params.bold}<b>{/if}
				    {if $params.link && !$params.customCode}
						<{$pageData.tag.$id[$params.ACLTag]|default:$pageData.tag.$id.MAIN} title='{$rowData.DESCRIPTION}' class='{if $params.dynamic_module}{$rowData[$params.dynamic_module]}{else}{$params.module|default:$pageData.bean.moduleDir}{/if}' href='index.php?action={$params.action|default:'DetailView'}&module={if $params.dynamic_module}{$rowData[$params.dynamic_module]}{else}{$params.module|default:$pageData.bean.moduleDir}{/if}&record={$rowData[$params.id]|default:$rowData.ID}&offset={$pageData.offsets.current+$smarty.foreach.rowIteration.iteration}&stamp={$pageData.stamp}'>
					{/if}
...
<script>
$(function(){
	$("body").on("change",".TP_between_select",function(e){
		e.preventDefault();
		if($(this).val() == 'TP_between_days'){
			$(this).next().show();
		}else{
			$(this).next().hide();
		}
