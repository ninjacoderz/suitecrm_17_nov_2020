...
class SugarWidgetFieldDateTime extends SugarWidgetReportField
{
    ...
    //thienpb code
    public function queryFilterTP_between_days(& $layout_def)
    {
        global $timedate;
        $begin = $this->now()->get($layout_def['start_plus_div'].$layout_def['between_start'].' days')->get_day_begin();
        $end = $this->now()->get($layout_def['end_plus_div'].$layout_def['between_end'].' days')->get_day_end();
        return $this->get_start_end_date_filter($layout_def, $begin->asDb(), $end->asDb());
    }
    ...
    public function displayInput($layout_def)
    {
        ...
        $filterTypes = array(' '                 => $app_strings['LBL_NONE'],
                             ...
                             'TP_between_days'  => 'Between'
                             );
        //thienpb custom
        $cal_dateformat = $timedate->get_cal_date_format();
        $str = "<select class='TP_between_select' name='type_{$layout_def['name']}'>";
        $str .= get_select_options_with_id($filterTypes, (empty($layout_def['input_name0']) ? '' : $layout_def['input_name0']));
        $plus_div = array('plus'=>'+','div'=>'-');
        $str .= "</select>";
        $str .= "<div id='{$layout_def['name']}_TP_between'  style='display:".($layout_def['input_name0']['type'] != 'TP_between_days' ? 'none' : 'block')."'>
                From:<br />
                <select class='TP_plus_div' style='padding: 0px 20px 0 5px;background-size: 30px 52px;height: 30px;' name='sl_start_{$layout_def['name']}_TP_plus_div'>";
        $str .= get_select_options_with_id($plus_div, (empty($layout_def['input_name0']['start_plus_div']) ? '' : $layout_def['input_name0']['start_plus_div']));
        $str .= "</select>
                <input style='width:40px;' type='text' name='{$layout_def['name']}_TP_between_start' id='{$layout_def['name']}_TP_between_start' value='{$layout_def['input_name0']['between_start']}'> 
                previous days <br />To :<br />
                <select class='TP_plus_div' style='padding: 0px 20px 0 5px;background-size: 30px 52px;height: 30px;' name='sl_end_{$layout_def['name']}_TP_plus_div'>";
        $str .= get_select_options_with_id($plus_div, (empty($layout_def['input_name0']['end_plus_div']) ? '' : $layout_def['input_name0']['end_plus_div']));
        $str .="</select>
                <input style='width:40px;' type='text' name='{$layout_def['name']}_TP_between_end' id='{$layout_def['name']}_TP_between_end' value='{$layout_def['input_name0']['between_end']}'>
                days after
            </div>";
        return $str;
    }
