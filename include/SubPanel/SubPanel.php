...
class SubPanel
{
    ...
    public function ProcessSubPanelListView($xTemplatePath, &$mod_strings, $countOnly = false)
    {
        ...
        if($this->subpanel_id == "pe_warehouse_pe_stock_items_1" && isset($_REQUEST['serial_number_filter'])) {
            $this->subpanel_defs->panel_definition['where'] = '(pe_stock_items_cstm.aos_invoices_id_c = "" OR pe_stock_items_cstm.aos_invoices_id_c IS NULL) AND pe_stock_items.serial_number LIKE "%' .trim($_REQUEST['serial_number_filter']) .'%" ';
        }
