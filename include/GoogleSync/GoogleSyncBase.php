...
class GoogleSyncBase
{
    ...
    protected function getUserMeetings($userId)
    {
        ...
        //BinhNT do tweak
        $query = "SELECT m.id FROM meetings m 
        LEFT JOIN meetings_users mu ON mu.meeting_id = m.id
        WHERE (assigned_user_id = '" . $userId . "' OR created_by  = '" . $userId . "' OR mu.user_id = '" . $userId . "') AND date_start <= now() + interval 3 month
        AND date_start >= now() - interval 2 DAY
        ";
