                    ...
                    if ($usage == 'query_only' && !empty($list_field['force_query_only_display'])) {
                        ...
                        if ($usage != 'query_only') {
                            ...
                            if (isset($aItem->field_defs[strtolower($list_field['name'])])) {
                                ...
                                //dung code - custom edit link in subpanel
                                if($list_field['module'] != '' && $list_field['fields']['ID'] != '') {
                                    $custom_edit_link = '<a href="' .'?module='.$list_field['module'].'&action=EditView&record='.$list_field['fields']['ID'] .'">[E]</a>';
                                    if( $list_field['module'] == 'Emails') {
                                        $return_module = $this_subpanel->parent_bean->module_dir;
                                        $return_id = $this_subpanel->parent_bean->id;
                                        $custom_edit_link .=  ' <a target="_blank" href="/index.php?module=Emails&action=ComposeViewWithPdfTemplate&return_action=DetailView&record='.$list_field['fields']['ID'] .'&return_id='.$return_id .'&return_module='.$return_module .'">[Compose]</a>';
                                    }
                                    $widget_contents[$aVal]['custom_edit_view'] = $custom_edit_link;
                                }   
                                if ($tmpField != null) {
