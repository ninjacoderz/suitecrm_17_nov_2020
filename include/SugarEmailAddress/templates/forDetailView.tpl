...
$(document).ready(function(){
	function render_link_gsearch(){
        $("#email1_span").each(function(k,e){
            $(this).find('.email-link-gsearch').remove();
           var emailAddress =  $(this).find('a[class="email-link"]').attr('data-email-address');
           if(emailAddress != ''){
            $(this).find('a[class="email-link"]').closest('td').after('<br><a style="color:blue;" class="email-link-gsearch"  target="_blank" href="https://mail.google.com/#search/'+emailAddress+'">GSearch </a>');
           }
        })
    }
    render_link_gsearch();
