...
$(document).ready(function(){
	function render_link_gsearch(){
        $(".email-address-input-group").each(function(k,e){
            $(this).find('.email-link-gsearch').remove();
           var emailAddress =  $(this).find('input[type="email"]').val();
           if(emailAddress != ''){
            $(this).closest('.email-address-input-group').prepend('<a style="color:blue;" class="email-link-gsearch"  target="_blank" href="https://mail.google.com/#search/'+emailAddress+'">GSearch </a>');

			var html_icon_copy_email_address = '<a class="copy-email-link" data-email-address="'+emailAddress+'" \
		   	...
		   	$(this).closest('.email-address-input-group').after(html_icon_copy_email_address);
		   }
        })
    }
    render_link_gsearch();
