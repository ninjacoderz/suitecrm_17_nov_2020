<?php
    ...
    switch ($state) {
        case 'download':
            ...
            shell_exec('AWS_ACCESS_KEY_ID='.$GLOBALS['AWS_ACCESS_KEY_ID'].' AWS_SECRET_ACCESS_KEY='.$GLOBALS['AWS_SECRET_ACCESS_KEY'].' aws s3 cp '.$GLOBALS['RECURSIVE'].' s3://'.$GLOBALS['BUCKET_NAME'].$folderName.$fileName.' '.$GLOBALS['FOLDER_NAME'].$folderName.$fileName);
            break;
        case 'upload':
    ...
    function upload($folderName,$fileName){
        if($is_today == 'yes'){
            ...
            shell_exec('AWS_ACCESS_KEY_ID='.$GLOBALS['AWS_ACCESS_KEY_ID'].' AWS_SECRET_ACCESS_KEY='.$GLOBALS['AWS_SECRET_ACCESS_KEY'].' aws s3 cp '.$GLOBALS['RECURSIVE'].' '.$GLOBALS['FOLDER_NAME'].$folderName.$fileName.' s3://'.$GLOBALS['BUCKET_NAME'].'/'.$folderName.$fileName);
        }
    }
    die();
?>