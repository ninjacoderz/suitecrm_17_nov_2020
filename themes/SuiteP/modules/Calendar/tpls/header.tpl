...
{/if}
 {* custom code -- button show all calendar *}
<div class="row {if $controls}monthHeader{/if}">
	<div class="col-xs-1" style="background-color:#bfcad3;">{$previous}</div>
	<div class="col-xs-10 text-center">
		<h3 class="custom-title-header"></h3>
		<button type="button" id="inset_date_dashlet_trigger" class="btn btn-danger" onclick="return false;">
			<span class="suitepicon suitepicon-module-calendar" alt="Enter Date"></span>
		</button>
		<button type="button" class="button primary" id="show_all_calendar" > <span class="glyphicon hidden glyphicon-refresh glyphicon-refresh-animate"></span>
			Show All Calendar
		</button>
		<input class="inset_date_dashlet" hidden autocomplete="off" type="text" name="inset_date_dashlet" id="inset_date_dashlet" value="" title="" tabindex="0" size="11" maxlength="10">
	<script type="text/javascript">
		{literal}		
			Calendar.setup ({
			inputField : "inset_date_dashlet",
			ifFormat : "%d/%m/%Y %H:%M",
			daFormat : "%d/%m/%Y %H:%M",
			button : "inset_date_dashlet_trigger",
			singleClick : true,
			dateStr : "",
			startWeekday: 0,
			step : 1,
	...
	<div class="col-xs-1 text-right" style="background-color:#bfcad3;">{$next}</div>
	<br>
</div>
