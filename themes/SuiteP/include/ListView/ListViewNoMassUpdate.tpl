...
{include file='include/ListView/ListViewPagination.tpl'}
	...
	<tr height='20'>
		...
		{foreach from=$displayColumns key=colHeader item=paramsCol}
		{* ThienPB code *}
            {if $colCounter == '1'}<th scope='col' width='{$paramsCol.width}%' data-toggle="true">
            {else}<th scope='col' width='{$paramsCol.width}%' data-hide="phone,tablet">{/if}
				<span sugar="sugar{$colCounter}"><div style='white-space: nowrap; width:100%; text-align:{$paramsCol.align|default:'left'}'>
                {if $paramsCol.sortable|default:true}
	                <a href='{$pageData.urls.orderBy}{$paramsCol.orderBy|default:$colHeader|lower}' class='listViewThLinkS1' title="{$arrowAlt}">{sugar_translate label=$paramsCol.label module=$pageData.bean.moduleDir}&nbsp;&nbsp;
					{if $paramsCol.orderBy|default:$colHeader|lower == $pageData.ordering.orderBy}
				...
				{else}
					{sugar_translate label=$paramsCol.label module=$pageData.bean.moduleDir}
				{/if}
	...
	{foreach name=rowIteration from=$data key=id item=rowData}
		...
		<tr height='20' class='{$_rowColor}S1'>
			...
			{foreach from=$displayColumns key=col item=params}
				<td scope='row' align='{$params.align|default:'left'}' {if $params.nowrap}nowrap='nowrap' {/if}valign='top'><span sugar="sugar{$colCounter}b" {if $col == 'DATE_CLOSED' && $pageData.bean.moduleDir == 'Opportunities'} field="{$col|lower}" type="{$displayColumns.$col.type}" class="{$hide} {if $inline_edit && ($displayColumns.$col.inline_edit == 1 || !isset($displayColumns.$col.inline_edit))}inlineEdit{/if}{if ($params.type == 'teamset')}nowrap{/if}" current-module="{$pageData.bean.moduleDir}" record-id="{$rowData.ID}"{/if}>
