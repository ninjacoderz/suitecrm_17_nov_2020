<?php
// created: 2018-10-08 20:23:14
$dictionary["pe_warehouse_log_pe_warehouse_log_1"] = array (
  'true_relationship_type' => 'one-to-one',
  'from_studio' => true,
  'relationships' => 
  array (
    'pe_warehouse_log_pe_warehouse_log_1' => 
    array (
      'lhs_module' => 'pe_warehouse_log',
      'lhs_table' => 'pe_warehouse_log',
      'lhs_key' => 'id',
      'rhs_module' => 'pe_warehouse_log',
      'rhs_table' => 'pe_warehouse_log',
      'rhs_key' => 'id',
      'relationship_type' => 'many-to-many',
      'join_table' => 'pe_warehouse_log_pe_warehouse_log_1_c',
      'join_key_lhs' => 'pe_warehouse_log_pe_warehouse_log_1pe_warehouse_log_ida',
      'join_key_rhs' => 'pe_warehouse_log_pe_warehouse_log_1pe_warehouse_log_idb',
    ),
  ),
  'table' => 'pe_warehouse_log_pe_warehouse_log_1_c',
  'fields' => 
  array (
    0 => 
    array (
      'name' => 'id',
      'type' => 'varchar',
      'len' => 36,
    ),
    1 => 
    array (
      'name' => 'date_modified',
      'type' => 'datetime',
    ),
    2 => 
    array (
      'name' => 'deleted',
      'type' => 'bool',
      'len' => '1',
      'default' => '0',
      'required' => true,
    ),
    3 => 
    array (
      'name' => 'pe_warehouse_log_pe_warehouse_log_1pe_warehouse_log_ida',
      'type' => 'varchar',
      'len' => 36,
    ),
    4 => 
    array (
      'name' => 'pe_warehouse_log_pe_warehouse_log_1pe_warehouse_log_idb',
      'type' => 'varchar',
      'len' => 36,
    ),
  ),
  'indices' => 
  array (
    0 => 
    array (
      'name' => 'pe_warehouse_log_pe_warehouse_log_1spk',
      'type' => 'primary',
      'fields' => 
      array (
        0 => 'id',
      ),
    ),
    1 => 
    array (
      'name' => 'pe_warehouse_log_pe_warehouse_log_1_ida1',
      'type' => 'index',
      'fields' => 
      array (
        0 => 'pe_warehouse_log_pe_warehouse_log_1pe_warehouse_log_ida',
      ),
    ),
    2 => 
    array (
      'name' => 'pe_warehouse_log_pe_warehouse_log_1_idb2',
      'type' => 'index',
      'fields' => 
      array (
        0 => 'pe_warehouse_log_pe_warehouse_log_1pe_warehouse_log_idb',
      ),
    ),
  ),
);