...
$phone_info ="";
$electrician_contact = BeanFactory::getBean("Contacts", $invoice->contact_id3_c);

if($electrician_contact->phone_mobile != ""){
    $phone_info .= "M: ". $electrician_contact->phone_mobile;
    $phone_info .= " ";
}
if($electrician_contact->phone_home != ""){
    $phone_info .= "H: ". $electrician_contact->phone_home;
    $phone_info .= " ";
}
if($electrician_contact->phone_work != ""){
    $phone_info .= "W: ". $electrician_contact->phone_work;
    $phone_info .= " ";
}
...
$temp_request = array(
    ...
    "aos_invoices_contact_id4_c" =>$invoice->electrician_contact_c,
    "aos_invoices_billing_contact" => $invoice->billing_contact,
    "aos_invoices_install_address_c" =>  $invoice->billing_address_street . " " 
                                        .  $invoice->billing_address_city . " "
                                        .  $invoice->billing_address_state. " "
                                        .  $invoice->billing_address_postalcode,
    "aos_invoices_contact_id3_c" => $phone_info,
    "aos_invoices_plumbing_notes_c" => $invoice->electrical_notes_c,
    "distance_to_suite_c" => str_replace(" km","",$invoice->distance_to_suitecrm_c),
);
$emailBean = new Email();
...
$url_calendar = 'https://calendar.pure-electric.com.au/#/installation-booking/'.$invoice->installation_calendar_id_c.'/electrician/'.$account_id;
...
//VUT - S - Copy SMS message = Email message
    /**Get Contact electrician */
    $electrician  = new Contact();
    $electrician->retrieve($invoice->contact_id_c);
$phone_number = preg_replace("/^0/", "+61", preg_replace('/\D/', '', $electrician->phone_mobile));
