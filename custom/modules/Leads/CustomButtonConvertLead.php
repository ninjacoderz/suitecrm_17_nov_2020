...
$account->save();
    ...
    if($type_button == 'convert_sanden_button'){
        ...
        $data_plumbing = json_decode(cusomFilterPlumberButtonConvert($type = 'plumber', $from_address), true);
        $data_electrician = json_decode(cusomFilterPlumberButtonConvert($type = 'electrician', $from_address), true);
        // Solve suggest
        $data_distance_plumbing = bubble_SortButtonConvert($data_plumbing);
        $data_distance_electrician = bubble_SortButtonConvert($data_electrician);
    ...
    if($type_button == 'convert_daikin_button'){
        ...
        while ($row = $db->fetchByAssoc($ret))
        {
            ...
            if($check_install_address_postalcode_VEEC < 3000 && $row['id'] == 'cbfafe6b-5e84-d976-8e32-574fc106b13f') {
                ...
                //custom code for products "Daikin Supply and Installation" --- Grand Total US7 2.5 fully installed = $3190
                else if($row['id'] =='79656166-ca6d-2715-2788-5d7ec2db2ce2'){
                    $product_line->product_qty = 1;
    ...
    if($type_button == 'convert_solar_button' && $product_type == "quote_type_solar"){
        ...
        $quote = convert_info_basic_quote($quote,$bean ,$contact ,$account);
        if( isset($_REQUEST['from_upload_solar']) && $_REQUEST['from_upload_solar'] == "solar"){
        ...
        // $quote->solargain_lead_number_c = 178562;
    ...
    .'" ,create_grid_button_number_c="'   .$bean->create_grid_button_number_c
    ...
    //Step 5 -- install convert 
    $result_data = array(
        'json_open_new_tag' => $json_open_new_tag,
        'array_id_opportunity_and_quote' => array(
            'create_opportunity_number_c' => $bean->create_opportunity_number_c,
            'create_solar_number_c' =>$bean->create_solar_number_c,
            'create_methven_number_c'  =>$bean->create_methven_number_c,
            'create_grid_button_number_c'  =>$bean->create_grid_button_number_c,
...
function cusomFilterPlumberButtonConvert($type, $f_address) {
    $db = DBManagerFactory::getInstance();
    if( $type == "plumber"){
...
function bubble_SortButtonConvert($distance_array )  
{  
    do  
    {  
        $swapped = false;  
