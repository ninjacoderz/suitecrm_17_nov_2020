...
$(document).ready(function () {
    ...
    //thienpb code
    $("#get_files_from_s3").after('<button type="button" style="background: #008000;" id="get_all_file_from_lead" class="button primary" title="Get Files From Lead">Get Files From Lead<span class="glyphicon hidden glyphicon-refresh glyphicon-refresh-animate"></span> </button>');
    $("#get_all_file_from_lead").click(function () {
        SUGAR.ajaxUI.showLoadingPanel();
        var quote_id = $('input[name="record"]').val();
        if (quote_id == '') {
            alert('Not have number ID Quotes. Please save and edit before.');
        ...
        var lead_id = $("input[name='leads_aos_quotes_1leads_ida']").val()
        if (lead_id == '') {
            alert('This Quote doesn\'t relate to Lead.');
            SUGAR.ajaxUI.hideLoadingPanel();
            return;
        }
        var pre_install_photos_c = $("input[name='pre_install_photos_c']").val();
        ...
        $.ajax({
            url: '/index.php?entryPoint=APICloneFile&method=clone_file_Lead_to_Quote&leadID=' + lead_id + '&quoteID=' + quote_id,
            success: function (data) {
                $(".files").empty();
                $.ajax({
                    url: $('#fileupload').fileupload('option', 'url'),
...
function generateCustomerLink(){
    ...
    const lead_id = $('#leads_aos_quotes_1leads_ida').val();
