...
//create new items 
        ...
        if(!in_array($row['id'],$array_id_part_numbers_not_use)) {
            ...
            if($row['part_number'] == 'STC Rebate Certificate'){
                ...
                if((int)$array_stc_veec['veec'] == '' || (int)$array_stc_veec['veec'] == 0 || (int)$array_stc_veec['veec'] == -1) {
                    ...
                    $product_line->product_list_price = ((int)$array_stc_veec['veec'] -1)*(-1);
                } 
            ...
            if($row['part_number'] == 'STC Rebate Certificate' || $row['part_number'] == 'VEEC Rebate Certificate'){
                $price_veec_stc += $product_line->product_total_price;
                // $price_veec_stc += -1023;
            } elseif($row['part_number'] == 'SV_SHWR'){
                $priceHotWater += $product_line->product_total_price;
            } elseif($row['part_number'] == 'SV_SHWR'){
            ...
            if($row['part_number'] == 'STC Rebate Certificate'){
                ...
                if((int)$array_stc_veec['veec'] == '' || (int)$array_stc_veec['veec'] == 0 || (int)$array_stc_veec['veec'] == -1) {
                    ...
                    $product_line->product_list_price = ((int)$array_stc_veec['veec'] -1)*(-1);
                    $product_line->product_unit_price = ((int)$array_stc_veec['veec'] -1)*(-1);
                }
            }elseif($row['part_number'] == 'SV_SHWR'){
                $product_line->product_qty =1; 
                $product_line->product_list_price = $row['cost'];
                ...
                $product_line->product_unit_price = 0;
    ...
    $discount_amount = 0;
    $price_include_admin_20 = round(($price_include_admin + $price_state) + ($price_include_admin + $price_state)*0.18,2);
    $gst = round($price_include_admin_20*0.1,2);
    $subTotal = $price_include_admin_20 + $price_veec_stc + $priceHotWater;
    $old_group_total = round($gst + $price_include_admin_20 + $price_veec_stc + $priceHotWater,2);
    $group_tmp = explode('.',round($gst + $price_include_admin_20 + $price_veec_stc + $priceHotWater,2));
    $new_group_total = '';
    $group_price_1 = '';
    ...
    $subTotal =  round($price_include_admin_20 + $price_veec_stc + $priceHotWater,2);
