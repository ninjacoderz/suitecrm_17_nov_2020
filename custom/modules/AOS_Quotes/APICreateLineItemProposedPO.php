...
if ($quote_id != '' && isset($quote_id)) {
    ...
    //Electrician
    if ($po_type == 'sanden_electrician' && $focus->id !='') {
        $db = DBManagerFactory::getInstance();
        //delete Old line_group
        $sql_delete_group = "   UPDATE aos_line_item_groups lig 
                                SET lig.deleted = 1 
        ...
        $html .= '<input type="hidden" name="electrician_vathidden" id="electrician_vathidden" value="'.get_select_options_with_id($app_list_strings['vat_list'], '').'">
                  ...
                  //Create default Sanden Electrician Installer       
        /**Create AOS_Line_Item_Groups */ 
        $row['id'] = "";
        $row['name'] = 'Sanden Install';
        $row['currency_id'] = '-99';
        ...
        $row['parent_id'] = $focus->id;
        $row['parent_include'] = 'AOS_Quotes';
        $row['po_type'] = 'sanden_electrician';

        $group_po_electrician = new AOS_Line_Item_Groups();
        $group_po_electrician->populateFromRow($row);
        $group_po_electrician->save();
        /**Sanden Electrician Products - Default */
        $part_numners = array(
        ...
        foreach($return_product as $product){
            ...
            $row['product_total_price'] = format_number($product['product_product_cost_price']);
            $row['vat'] = "10.0";
            $row['group_id'] = $group_po_electrician->id;
            $row['product_id'] = $product['product_product_id'];
            $total_price += $product['product_product_cost_price'];
            $prod_invoice = new AOS_Products_Quotes();
            $prod_invoice->populateFromRow($row);
        ...
        $focus->electrician_total_amt = format_number($total_price);
        $focus->electrician_subtotal_amount = format_number($total_price);
        $focus->electrician_tax_amount = format_number($total_price/10);
        $focus->electrician_total_amount = format_number($total_price + $total_price/10);
        $focus->save();
