...
//info login
  ...
  if($data_request["assigned_user_id"] == '8d159972-b7ea-8cf9-c9d2-56958d05485e'){
    ...
    $pass = urlencode('Podled0i1!');
  }else{
    $username = urlencode('paul.szuster@solargain.com.au');
    $pass = urlencode('Edgecumbe2016*');
  }
...
//objectHome
  ...
  $ch = curl_init();
  curl_setopt($ch, CURLOPT_URL, 'https://solarvic.force.com/industry/s/sfsites/aura?r=2&ui-comm-runtime-components-aura-components-siteforce-controller.PubliclyCacheableComponentLoader.getPageComponent=1');
  curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
...
//
if($data_request['process'] == 'create'){
...
//check this quote existed
    ...
    array (
      ...
      array (
        ...
        array (
          'listReference' => 
          array (
            'entityKeyPrefixOrApiName' => 'Installation__c',
            'listViewIdOrName' => '00B0o00000EWLsvEAH',
      ...
      array (
        ...
        'params' => 
        array (
          'filterName' => '00B0o00000EWLsvEAH',
          'entityName' => 'Installation__c',
          'pageSize' => 50,
          'layoutType' => 'LIST',
          'sortBy' => NULL,
          'getCount' => false,
          ...
          'offset' => 0,
          'searchString' => $data_request['slv_quote_sg_number_c'],
        ),
        'storable' => true,
      ),
    ),
  ...
  $ch = curl_init();
  curl_setopt($ch, CURLOPT_URL, 'https://solarvic.force.com/industry/s/sfsites/aura?r=8&ui-force-components-controllers-lists-listViewDataManager.ListViewDataManager.getItems=1&ui-force-components-controllers-lists-listViewManagerGrid.ListViewManagerGrid.getRecordLayoutComponent=1');
  curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
  curl_setopt($ch, CURLOPT_POSTFIELDS, "message=".urlencode(json_encode($check_quote))."&aura.context=%7B%22mode%22%3A%22PROD%22%2C%22fwuid%22%3A%22".$fwuid."%22%2C%22app%22%3A%22siteforce%3AcommunityApp%22%2C%22loaded%22%3A%7B%22APPLICATION%40markup%3A%2F%2Fsiteforce%3AcommunityApp%22%3A%22".$communityApp."%22%2C%22COMPONENT%40markup%3A%2F%2FforceCommunity%3AobjectHome%22%3A%22".$objectHome."%22%2C%22COMPONENT%40markup%3A%2F%2Fforce%3AoutputField%22%3A%22".$outputField."%22%7D%2C%22dn%22%3A%5B%5D%2C%22globals%22%3A%7B%7D%2C%22uad%22%3Afalse%7D&aura.pageURI=%2Findustry%2Fs%2Fquotes&aura.token=".$token);
  curl_setopt($ch, CURLOPT_POST, 1);
  curl_setopt($ch, CURLOPT_ENCODING, 'gzip, deflate');
  set_cookie($ch,$tmpfname);
  $headers = array();
  ...
  $headers[] = 'Accept-Encoding: gzip, deflate, br';
  $headers[] = 'Accept-Language: en-US,en;q=0.9';
  $headers[] = 'X-Sfdc-Request-Id: 2399173000030b36f6';
  ...
  $recordId_check = '';
  for($i = 0; $i < count($result_decode->actions) ; $i++){
    if(isset($result_decode->actions[$i]->returnValue->recordIdActionsList)){
      if(count($result_decode->actions[$i]->returnValue->recordIdActionsList) > 0){
        $recordId_check = $result_decode->actions[$i]->returnValue->recordIdActionsList[0]->recordId;        
      }
    }
  }

  if($recordId_check == ''){
    $check_quote['actions'][0]['params']['listReference']['listViewIdOrName'] = '00B0o00000EWLssEAH';
    $check_quote['actions'][1]['params']['filterName'] = '00B0o00000EWLssEAH';

    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, 'https://solarvic.force.com/industry/s/sfsites/aura?r=9&ui-force-components-controllers-lists-listViewDataManager.ListViewDataManager.getItems=1&ui-force-components-controllers-lists-listViewManagerGrid.ListViewManagerGrid.getRecordLayoutComponent=1');
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
    curl_setopt($ch, CURLOPT_POSTFIELDS, "message=".urlencode(json_encode($check_quote))."&aura.context=%7B%22mode%22%3A%22PROD%22%2C%22fwuid%22%3A%22BdBBmM1TQup8KFEuAp9w1Q%22%2C%22app%22%3A%22siteforce%3AcommunityApp%22%2C%22loaded%22%3A%7B%22APPLICATION%40markup%3A%2F%2Fsiteforce%3AcommunityApp%22%3A%22".$communityApp."%22%2C%22COMPONENT%40markup%3A%2F%2Fforce%3ArecordLayoutBroker%22%3A%22".$recordLayoutBroker."%22%2C%22COMPONENT%40markup%3A%2F%2FforceCommunity%3AobjectHome%22%3A%22".$objectHome."%22%2C%22COMPONENT%40markup%3A%2F%2Fforce%3AoutputField%22%3A%22".$outputField."%22%7D%2C%22dn%22%3A%5B%5D%2C%22globals%22%3A%7B%7D%2C%22uad%22%3Afalse%7D&aura.pageURI=%2Findustry%2Fs%2Finstallation%2FInstallation__c%2F00B0o00000EWLssEAH&aura.token=".$token);
    curl_setopt($ch, CURLOPT_POST, 1);
    curl_setopt($ch, CURLOPT_ENCODING, 'gzip, deflate');
    set_cookie($ch,$tmpfname);
    $headers = array();
    ...
    $headers[] = 'Content-Type: application/x-www-form-urlencoded; charset=UTF-8';
    $headers[] = 'Accept: */*';
    $headers[] = 'Referer: https://solarvic.force.com/industry/s/installation/Installation__c/00B0o00000EWLssEAH';
    $headers[] = 'Connection: keep-alive';
    curl_setopt($ch, CURLOPT_HTTPHEADER, $headers);
    $result = curl_exec($ch);
    curl_close($ch);
  ...
  if($recordId_check != ''){
    ...
    // get logic status
      ...
      for($i = 0; $i < count($globalValueProviders) ; $i++){
        if($globalValueProviders[$i]->type == '$Record'){
          $status = $globalValueProviders[$i]->values->records->{$recordId}->Installation__c->record->fields->Status__c->displayValue;
        }
      }
    //
    ...
    $data_return['error'] = 'The Solargain Quote number is existed';
  ...
  $recordId = explode("/",$data_request['solarvicID']);
  $recordId = $recordId[0];
}
//

//$recordId = "a060o00001TtI0ZAAV";
...
//
    ...
    switch($data_request['slv_panel_type_c']){
      ...
      case 'Jinko 315W Cheetah Mono PERC':
        ...
        break;
      case 'Jinko 300W Mono PERC':
        $panel_type_suite = "JKM300M-60";
        break;
      case 'LG NeON 2 345W':
    ...
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, 'https://solarvic.force.com/industry/s/sfsites/aura?r=10&other.NewQuote.saveInstallation=1&other.NewQuote.saveInstalledProducts=1');
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
    curl_setopt($ch, CURLOPT_POSTFIELDS, "message=".urlencode(json_encode($data_mess_string))."&aura.context=%7B%22mode%22%3A%22PROD%22%2C%22fwuid%22%3A%22".$fwuid."%22%2C%22app%22%3A%22siteforce%3AcommunityApp%22%2C%22loaded%22%3A%7B%22APPLICATION%40markup%3A%2F%2Fsiteforce%3AcommunityApp%22%3A%22".$communityApp."%22%2C%22COMPONENT%40markup%3A%2F%2FforceCommunity%3ArecordDetail%22%3A%22".$recordDetail."%22%2C%22COMPONENT%40markup%3A%2F%2FforceCommunity%3ArelatedRecords%22%3A%22".$relatedRecords."%22%2C%22COMPONENT%40markup%3A%2F%2Fforce%3AoutputField%22%3A%22".$outputField."%22%2C%22COMPONENT%40markup%3A%2F%2FforceCommunity%3AobjectHome%22%3A%22".$objectHome."%22%7D%2C%22dn%22%3A%5B%5D%2C%22globals%22%3A%7B%22density%22%3A%22VIEW_ONE%22%7D%2C%22uad%22%3Afalse%7D&aura.pageURI=%2Findustry%2Fs%2Finstallation%2F".$recordId."%2F".$recordName."&aura.token=".$token);
    curl_setopt($ch, CURLOPT_POST, 1);
    curl_setopt($ch, CURLOPT_ENCODING, 'gzip, deflate');
    curl_setopt($ch, CURLOPT_COOKIEJAR, $tmpfname);
    curl_setopt($ch, CURLOPT_COOKIEFILE, $tmpfname);
